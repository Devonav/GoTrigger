...
/**
 * Vivaldi CSV Importer
 * Vivaldi uses the same CSV format as Chrome (both are Chromium-based)
 * CSV Format: name,url,username,password,note
 */
export class VivaldiCsvImporter extends BaseImporter {
  readonly formatId = 'vivaldi-csv';
  readonly formatName = 'Vivaldi (CSV)';
  readonly formatDescription = 'CSV export from Vivaldi browser';
  ...
  async parse(content: string): Promise<ParsedCredential[]> {
    ...
    for (const row of rows) {
      try {
        ...
        console.warn('Error parsing Vivaldi CSV row:', error);
        continue;
      }
    }
